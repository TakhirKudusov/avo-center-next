...
const createUniqueRange = (number: string, startingId: string) =>
  Array.from(Array(number).keys()).map((key) => ({
    value: key + 1,
    id: `${startingId}-${key}`,
  }));

const isSameDay = (day1: TDate, day2: TDate) => {
...
const putZero = (number: number) =>
  number.toString().length === 1 ? `0${number}` : number;

const toExtendedDay = (date: TDate) => [date.year, date.month, date.day];

const shallowClone = (value: Object | Array<any>) => ({ ...value });

const deepCloneObject = (obj: Object | Array<any>) =>
  JSON.parse(
...
const getDateAccordingToMonth = (date: TDate, direction: string) => {
...
const hasProperty = (object: Object, propertyName: string) =>
  Object.prototype.hasOwnProperty.call(object || {}, propertyName);

const getValueType = (value: TDate | TDate[]) => {
